...
bool InstantaneousMixingModel<T>::updateNodeOutflow(HybridMixing<T>* sim, std::vector<Mixture<T>>& tmpMixtures) {
    ...
    for (auto& [nodeId, mixtureInflowList] : mixtureInflowAtNode) {
        ...
        // Constructor for placeholder dummy object. Does not increase mixtureCount.
...
template<typename T>
void InstantaneousMixingModel<T>::updateMixtures(T timeStep, arch::Network<T>* network, AbstractConcentration<T>* sim, std::unordered_map<size_t, std::shared_ptr<Mixture<T>>>& mixtures) {
...
template<typename T>
void InstantaneousMixingModel<T>::generateNodeOutflow(AbstractConcentration<T>* sim, std::unordered_map<size_t, std::shared_ptr<Mixture<T>>>& mixtures) {
    ...
    for (auto& [nodeId, mixtureInflowList] : mixtureInflowAtNode) {
        ...
        if ( !createMixture.at(nodeId)) {
            ...
            auto newMixture = sim->createMixture(newConcentrations);
...
template<typename T>
void InstantaneousMixingModel<T>::updateChannelInflow(T timeStep, AbstractConcentration<T>* sim, arch::Network<T>* network, std::unordered_map<size_t, std::shared_ptr<Mixture<T>>>& mixtures) {
            ...
            if ((channelFlowRate > 0.0 && tank->getNodeBId() == nodeId) || (channelFlowRate < 0.0 && tank->getNodeAId() == nodeId)) {
                ...
                if (mixtureOutflowAtNode.count(nodeId)) {
                    ...
                    if (tankMixtureIt != this->mixturesInEdge.end() && !tankMixtureIt->second.empty()) {
                        ...
                        auto newMixtureId = sim->createMixture(tankMixture->getSpecieConcentrations())->getId();
...
template<typename T>
void DiffusionMixingModel<T>::updateMixtures(T timeStep, arch::Network<T>* network, AbstractConcentration<T>* sim, std::unordered_map<size_t, std::shared_ptr<Mixture<T>>>& mixtures) {
...
template<typename T>
void DiffusionMixingModel<T>::generateInflows(T timeStep, arch::Network<T>* network, AbstractConcentration<T>* sim, std::unordered_map<size_t, std::shared_ptr<Mixture<T>>>& mixtures) {
...
std::tuple<std::function<T(T)>,std::vector<T>,T> DiffusionMixingModel<T>::getAnalyticalSolutionFunction(T channelLength, T channelWidth, int resolution, T pecletNr, const std::vector<FlowSectionInput<T>>& parameters, std::function<T(T)> fConstant) { 
    ...
    for (const auto& parameter : parameters) {
        ...
        for (int n = 1; n < resolution; n++) {
            ...
            for (size_t i = 0; i < parameter.segmentedResult.size(); i++) {
...
std::tuple<std::function<T(T)>,std::vector<T>,T> DiffusionMixingModel<T>::getAnalyticalSolutionTotal(
    ...
    T pecletNr, const std::vector<FlowSection<T>>& flowSections, std::unordered_map<size_t, std::shared_ptr<Mixture<T>>>& Mixtures) { 
